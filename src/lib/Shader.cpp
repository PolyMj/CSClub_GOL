...
// Print out vertex and fragment code
void printVertFragCode(string &vertexCode, string &fragCode) {
	printShaderCode(vertexCode, "VERTEX SHADER CODE");
	printShaderCode(fragCode, "FRAGMENT SHADER CODE");
}

// Print shader code with the given title
void printShaderCode(string &code, const char* title) {
	// Display header
	for (int i = -6; i < strlen(title); i++) {
		cout << "*";
	}
	...
	// Display end line
	for (int i = -10; i < strlen(title); i++) {
		cout << "*";
	}
	cout << endl << "** END " << title << " **" << endl;
}
...
// Create program from shader files
GLuint loadAndCreateShaderProgram(const string &vertFile, const string &fragFile, bool debug) {
	...
	// Print shader code
	if (debug) {
		printVertFragCode(vertexCode, fragCode);
	}
